...
const Chat = () => {
  ...
  // Fetch profile from API or use localStorage data if available
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const userData = localStorage.getItem('user'); // Check localStorage first
        if (userData) {
          setUser(JSON.parse(userData)); // If found, use localStorage data
        } else {
          ...
          if (response.status === 200) {
            ...
            console.log(response.data.user); // Debugging the user object
          }
        }
      } catch (error) {
  ...
  return (
    <div className="relative">
      ...
      <section
        className={`w-10 h-10 rounded-full md:hidden block xl:hidden 2xl:hidden sm:hidden lg:hidden fixed top-4 right-6 shadow-lg shadow-[#131313] hover:shadow-inner hover:shadow-gray-800 cursor-pointer ${open ? 'hidden' : 'block'}`}
        onClick={() => setOpen(!open)}
      >
        <img src={user?.avatar_url || '/avatar.png'} alt="" className="rounded-full" />
      </section>
      <section
        className={`w-10 h-10 rounded-full md:block hidden xl:block 2xl:block sm:block lg:block fixed bottom-6 left-6 shadow-lg shadow-[#131313] hover:shadow-inner hover:shadow-gray-800 cursor-pointer ${open ? 'hidden' : 'block'}`}
        onClick={() => setOpen(!open)}
      >
        <img src={user?.avatar_url || '/avatar.png'} alt="" className="rounded-full" />
      </section>
      ...
      <main className="contentBody h-[500px] ">
        {/* Feedback Form */}
        <FeedbackForm openFeedbackModal={openFeedbackModal} setOpenFeedbackModal={setOpenFeedbackModal} />
